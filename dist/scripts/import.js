...
const slugify_1 = __importDefault(require("../src/utils/slugify")); // <-- uses your util
const prisma = new client_1.PrismaClient();
const toBool = (v) => !!v && /^true$/i.test((v ?? '').trim());
const toNum = (v) => {
    if (v == null || v === '')
        return undefined;
    const n = Number(v);
...
const str = (r, k) => (r[k]?.trim() ? r[k].trim() : '');
// Accept ; > / | as separators
function splitCollectionPath(raw) {
    if (!raw)
        return [];
    return raw
        .split(/[;>\/|]+/g)
...
async function ensureParentCategory(name) {
    const parentSlug = (0, slugify_1.default)(name);
...
async function ensureChildCategory(childName, parentId, parentSlug) {
    const base = (0, slugify_1.default)(childName);
...
async function resolveCategoryIdFromCollection(collection) {
...
async function main() {
    const file = process.argv[2] || node_path_1.default.resolve(process.cwd(), 'products.csv');
    const csv = node_fs_1.default.readFileSync(file, 'utf8');
