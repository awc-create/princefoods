  ...
  return (
    <div className="fixed bottom-4 right-4 w-80 rounded-xl shadow-lg bg-white border">
      ...
      <div className="p-3 space-y-2 max-h-80 overflow-auto text-sm">
        {messages.map((m, i) => (
          <div key={i} className={m.role === 'user' ? 'text-right' : 'text-left'}>
            <div
              className={`inline-block px-3 py-2 rounded-lg ${m.role === 'user' ? 'bg-black text-white' : 'bg-gray-100'}`}
            >
              {m.content}
            </div>
          </div>
      ...
      <div className="p-2 flex gap-2">
        ...
        />
        <button onClick={send} className="px-3 py-1 rounded bg-black text-white text-sm">
          Send
        </button>
      </div>
    </div>
