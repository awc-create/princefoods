...
function CrownChatIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 64 64" aria-hidden="true">
      ...
      {/* bubble */}
      <path
        d="M12 36c0-8.8 9.2-16 20.5-16S53 27.2 53 36s-9.2 16-20.5 16c-2.8 0-5.5-.4-8-1.2L12 55l3.7-6.1c-2.3-3-3.7-6.5-3.7-10z"
        fill="currentColor"
      />
    </svg>
  );
...
export default function PrinceChat() {
  const [open, setOpen] = useState<boolean>(
    () => typeof window !== 'undefined' && localStorage.getItem('pf_chat_open') === '1'
  );
  const [threadId, setThreadId] = useState<string | null>(() =>
    typeof window !== 'undefined' ? localStorage.getItem('pf_thread_id') : null
  );
  ...
  function scheduleIdleClose() {
    ...
    idleTimerRef.current = window.setTimeout(async () => {
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'assistant', content: 'Chat closed due to inactivity. Start a new chat any time.' }
      ]);
          ...
          <div className={styles.history}>
            {messages.length === 0 && (
              <div className={styles.welcome}>
                👋 Hi! Ask about <strong>delivery, returns, frozen packing</strong>, or anything
                from our FAQs & policies.
              </div>
            )}
            {messages.map((m, i) => (
              <div
                key={i}
                className={`${styles.row} ${m.role === 'user' ? styles.right : styles.left}`}
              >
