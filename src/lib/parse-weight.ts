// src/lib/parse-weight.ts
export type ParsedWeight = { grams: number; matched: string } | null;
...
export function parseWeightToGrams(name: string | null | undefined): ParsedWeight {
  const s = String(name ?? '')
    .toLowerCase()
    .replace(/\s+/g, ' ')
    .trim();
  ...
  // 1) "2 x 500g" / "3×1kg"
  let m = s.match(
    /(\d+(?:\.\d+)?)\s*[x×]\s*(\d+(?:\.\d+)?)(\s*)(kg|g|ml|l|lt|liter|litre|ltrs|ltr)\b/
  );
  if (m) {
    const count = parseFloat(m[1]);
    const qty = parseFloat(m[2]);
    const unit = m[4] as keyof typeof UNIT_FACTOR;
  ...
  if (m) {
    ...
    const unit = m[2] as keyof typeof UNIT_FACTOR;
  ...
  if (m) {
    ...
    if (DISQUALIFIERS.some((w) => after.includes(w))) return null;
  }
