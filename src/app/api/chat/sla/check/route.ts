...
export async function GET(req: NextRequest) {
  ...
  if (key !== process.env.SLA_CRON_KEY) return NextResponse.json({ error: 'unauthorized' }, { status: 401 });
  ...
  for (const t of threads) {
    ...
    const latestUserMsg = [...t.messages].reverse().find(m => m.role === 'user')?.content ?? '(no user message)';
    const url = `${process.env.APP_BASE_URL}/admin/chat?threadId=${t.id}`;
    const minutesOver = Math.max(0, Math.round((now - start - slaMs) / 60000));

    await sendSlaEmailTemplate({ threadId: t.id, preview: latestUserMsg, adminUrl: url, minutesOverdue: minutesOver });

    await prisma.chatThread.update({ where: { id: t.id }, data: { emailEscalationSentAt: new Date() } });
    // optional courtesy note to customer:
    await prisma.chatMessage.create({
      data: { threadId: t.id, role: 'assistant', content: "Thanks for your message — we’ve emailed our support team to jump in. You’ll hear from us shortly." }
    });
