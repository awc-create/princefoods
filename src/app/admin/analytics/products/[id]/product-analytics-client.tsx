// src/app/admin/analytics/products/[id]/product-analytics-client.tsx
'use client';
...
/** Permissive prop shape for Recharts runtime-only import (avoids redux/recharts type clashes) */
interface AnyProps {
  [key: string]: unknown;
  children?: React.ReactNode;
}
interface RechartsModule {
  ResponsiveContainer: React.ComponentType<AnyProps>;
  LineChart: React.ComponentType<AnyProps>;
  Line: React.ComponentType<AnyProps>;
  XAxis: React.ComponentType<AnyProps>;
  YAxis: React.ComponentType<AnyProps>;
  CartesianGrid: React.ComponentType<AnyProps>;
  Tooltip: React.ComponentType<AnyProps>;
  Legend: React.ComponentType<AnyProps>;
}

export default function ProductAnalyticsClient({ id }: { id: string }) {
  const [loading, setLoading] = useState(true);
  const [product, setProduct] = useState<Product | null>(null);
  const [daily, setDaily] = useState<Daily[]>([]);
  const [metric, setMetric] = useState<Metric>('views');

  const R = useRef<RechartsModule | null>(null);
  const [hasCharts, setHasCharts] = useState(false);

  // Runtime import of recharts (no type imports) to keep TS happy
  useEffect(() => {
    let mounted = true;
    import('recharts')
      .then((m) => {
        if (!mounted) return;
        R.current = m as unknown as RechartsModule;
        setHasCharts(true);
      })
...
function Chart({
  ...
  R: React.MutableRefObject<RechartsModule | null>;
}) {
